...
const CountryDetailDescription = ({country}: Props) => {
  return (
      <div className="flex flex-col font-normal text-[#000]/70 text-[16px] gap-[10px] max-w-[886px]">
          <p>
          ...
          <p>{country?.flags?.alt}</p>
            ...
            <p>
              {`${
                country?.name.common
              } holds the status of a United Nations member and operates as a republic, emphasizing its commitment to democratic governance. The country's national demonym is "Indian," both for males and females. The FIFA code representing ${
              country?.name.common
                      } in international sports competitions is ${
              country?.car?.signs[0]
                      } and the country follows ${
              country?.car?.side
                      }-hand driving with vehicle registration signs marked as "${
              country?.car?.signs[0]
                      }".`}
            </p>
            <p>
              {`The time zone in ${country?.name.common} is ${
              country?.timezones
                      } and the official start of the week is on ${
              country?.startOfWeek
                      }. The geographical coordinates of the capital, ${
              country?.capital
                      }, are approximately 28.6° latitude and 77.2° longitude. The country's continent is ${
              country?.region
                      }, aligning with its regional placement in ${country?.subregion}.`}
            </p>
            <p>
              {`${
                country?.name.common
              ...
              country?.idd?.root
                      }${country?.idd?.suffixes.toString()}, signifying its global connectivity. With a Gini coefficient of ${Object.values(
              country?.gini
